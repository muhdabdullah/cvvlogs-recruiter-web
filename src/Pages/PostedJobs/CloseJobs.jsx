...
import { getClosedJobs } from "../../actions/alljobsAction";
import { getDeleteJob } from "../../actions/deleteJobAction";
import Modal from "react-modal";
import FullPageLoader from "../../Components/fullpageloader/fullPageLoader";

function CloseJobs(props) {
  ...
  const jobData = async (userId) => {
    await props.getClosedJobs(userId);
    return null;
  };
  const deletefunc = (e) => {
    ...
    <>
      ...
      <div className="container container-bottom padder">
        <div className="row mb-3">
          <div className="col-md-12">
            <h1 className="posted-jobs-rexr-head">Closed Jobs</h1>
            ...
            props.alljobsReducer.alljobs.jobs.length > 0 ? (
              props.alljobsReducer.alljobs.jobs.map((alljobs) => (
                <div className="job_result">
                  ...
                  <Link
                    to={`/PostedJobsDesc?id=${alljobs.id}&job_type=close`}
...
const mapDispatchToProps = (dispatch) => ({
  getClosedJobs: (userId) => dispatch(getClosedJobs(userId)),
  getDeleteJob: (userId, id) => dispatch(getDeleteJob(userId, id)),
});
export default connect(mapStateToProps, mapDispatchToProps)(CloseJobs);
